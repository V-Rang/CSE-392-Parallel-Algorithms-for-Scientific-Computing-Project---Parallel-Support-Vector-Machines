# called by mywork.py to "fit" and "optimize parameters" based on three cases:
# 1. cpu as is i.e. np.linalg.pinv
# 2. cpu_rsvd
# 3. gpu_rsvd
...
class LSSVC():
    ...
    def _optimize_parameters(self, X, y_values):
        ...
        Aa=np.copy(A)
        # f2 = open('A_array.txt','a')
        # np.savetxt(f2,[len(Aa)],fmt='%1.3f')
        # np.savetxt(f2,Aa)
        np.savetxt('A_array.txt',Aa)
        print(Aa.shape)
